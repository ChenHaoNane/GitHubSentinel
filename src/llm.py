...
class LLM:
    def __init__(self):
        ...
        with open("prompts/hackernews_report_prompt.txt", "r", encoding='utf-8') as file:
    ...
    def generate_hackernews_report(self, markdown_content, dry_run=False):
        ...
        if dry_run:
            # 如果启用了dry_run模式，将不会调用模型，而是将提示信息保存到文件中
            LOG.info("Dry run mode enabled. Saving prompt to file.")
            with open("hackernews/prompt.txt", "w+") as f:
                # 格式化JSON字符串的保存
                json.dump(messages, f, indent=4, ensure_ascii=False)
            LOG.debug("Prompt已保存到 hackernews/prompt.txt")

            return "DRY RUN"
        LOG.info("使用 deepseek-chat 模型开始生成报告。")
        try:
            # 调用OpenAI GPT模型生成报告
            response = self.client.chat.completions.create(
