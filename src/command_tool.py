...
from hackernews_client import HackernewsClient  # 从hackernews_client模块导入HackernewsClient类，用于Hacker News API操作
...
def main():
    ...
    hackernews_client = HackernewsClient()  # 创建Hacker News客户端实例
    llm = LLM()  # 创建语言模型实例
    ...
    command_handler = CommandHandler(github_client, subscription_manager, report_generator, hackernews_client)  # 创建命令处理器实例
